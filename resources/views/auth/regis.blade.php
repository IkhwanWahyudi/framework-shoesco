...
@section('content')
    <div class="w-screen h-screen relative">
        <div class="w-screen h-screen absolute flex flex-col justify-around items-center bg-[#000000ac]">
            ...
            <div class="h-[500px] w-[500px] flex flex-col justify-around text-center rounded-lg z-40 bg-[#ffffff68]">
                <form action="{{ route('register.action') }}" method="post" class="w-full flex flex-col items-center">
                    @csrf
                    @if(session('error'))
                    ...
                    <button type="submit" class="w-[200px] h-auto py-4 mt-16 text-white font-medium bg-[#5f7251] rounded-md flex justify-center items-center hover:bg-[#49573e]">Sign Up</button>
                </form>
                <div>
                    <a href="{{ route('login') }}" class="text-blue-500 hover:text-blue-900">Already have an account?</a>
                </div>
            </div>
        </div>
